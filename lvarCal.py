...
from huihui import (compute_all_hui_heubel, 
...
from mkt_day_end import get_last_30_days_market_value_avg
...
def quntileCal(csh: List[dict]):
 ...
 # Get unique security codes from client holdings
    ...
    for client in client_df["fintr_customer_id"].unique():
        ...
        weights = investor_returns.mul(
            client_data.set_index("security_code")["weight"], axis=1
        )
        port_returns = weights.sum(axis=1)
        # weighted_returns = investor_returns.mul(weights, axis=1)
    ...
    client_df = client_df.merge(quntile_results[["fintr_customer_id", "var_95"]], on="fintr_customer_id", how="left")
...
def huihuiCal(client_df,stock_df):
    ...
    outstanding_shares_df = pd.DataFrame(
        outstanding_shares,
        columns=["scrip", "outstanding_shares_mn", "floating_shares_mn", "floating_percent"]
    )
    floating_shares = dict(zip(outstanding_shares_df['scrip'], outstanding_shares_df['floating_shares_mn']))
